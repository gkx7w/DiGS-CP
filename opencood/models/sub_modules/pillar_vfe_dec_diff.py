...
class PillarVFE(nn.Module):
    ...
    def normalize_statistical_features(self, statistical_features):
        ...
        for i in range(3):
            ...
            # 检查最大值和最小值之间是否有足够的差异
            if max_val - min_val > 1e-10:  # 使用小阈值检测差异
                # 应用最大最小归一化，将数据缩放到 [0, 1] 范围
                normalized_features[:, i] = (channel_data - min_val) / (max_val - min_val)
        ...
        points_count = normalized_features[:, 3]
        ...
        if max_points > 0:
            ...
            if max_log > 0:
                normalized_features[:, 3] = log_points / max_log
