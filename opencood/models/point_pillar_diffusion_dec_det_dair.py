...
class PointPillarDiffusionDecDetDair(nn.Module):
    def __init__(self, args):
        super(PointPillarDiffusionDecDetDair, self).__init__()
    ...
    def get_processed_lidar(self, batch_dict):
        ...
        for batch_i in range(len(batch_dict['boxes_fused'])):
            ...
            # pc_range = [-100.8, -40, -3.5, 100.8, 40, 1.5] #dair
            ...
            for car_idx in range(len(pre_fused_boxes)):
                # 获取当前box中的点并平移到以box中心为原点的坐标系 box里没有点怎么办？？直接丢掉
            ...
            if len(box_coords_stack) == 0:
                ...
                processed_lidar = None # 此处只能丢弃整个batch
            else:
                processed_lidar = {
    ...
    def forward(self, batch_dict):
        ...
        output_dict['stage1_out'] = batch_dict['stage1_out']
