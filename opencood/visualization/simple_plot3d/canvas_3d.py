...
class Canvas_3D(object):
    ...
    def draw_canvas_points_color(self,
                           canvas_xy,
                           valid_mask,
                           radius=-1,
                           color_index = None,
        ...
        # color_list = ["#f23752", "#47b8e0", "#FF9000", "#7B68EE", "#C36C88"]
        color_list = [(197, 112, 139),(71, 184, 224),(255, 164, 0),(123, 104, 238),(65, 176, 73)]
        if color_index !=None:
            for mask,color in zip(color_index,color_list):
                select_mask = np.logical_and(valid_mask,mask)
                self.draw_canvas_points(canvas_xy[select_mask],colors=color)
        else:
            self.draw_canvas_points(canvas_xy[valid_mask])

    def draw_canvas_points_color_one(self,
                           canvas_xy,
                           radius=-1,
                           color_select = None,
                           colors=None,
        ...
        if color_select !=None:
            self.draw_canvas_points(canvas_xy,colors=color_list[color_select])
        else:
            self.draw_canvas_points(canvas_xy)
