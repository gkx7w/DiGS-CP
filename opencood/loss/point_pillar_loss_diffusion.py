...
class PointPillarLossDiffusion(nn.Module):
    def __init__(self, args):
        ...
        self.gamma = 2.0
        self.sigma_data = 0.1
        self.num_timesteps = args.get('num_timesteps', 1000)
        self.total_loss = 0
        self.diff_loss = 0
        self.rcnn_loss = 0
        self.cls_loss = 0
    ...
    def stage2_loss(self, rcnn_cls, rcnn_iou, rcnn_reg, tgt_cls, tgt_iou, tgt_reg):
        ...
        if hasattr(self, 'use_iou_guided_reg') and self.use_iou_guided_reg:
            iou_weights = torch.clamp(tgt_iou, 0.1, 1.0)
        ...
        if self.use_iou_loss == "adaptive":
            ...
            weights_iou[tgt_iou_normalized < -0.2] = 3  
            loss_iou = weighted_smooth_l1_loss(rcnn_iou, tgt_iou_normalized, weights=weights_iou).mean()
        
        else:
            weights_iou = tgt_cls.float()  
            loss_iou = weighted_smooth_l1_loss(rcnn_iou, tgt_iou_normalized, weights=weights_iou).mean()
    ...
    def cal_diff_loss(self, p_e_batch, g_e_batch):
        diff_loss = 0.0
    ...
    def forward(self, output_dict, target_dict):
        ...
        if 'pred_out' in output_dict.keys() and 'rcnn_label_dict' in output_dict.keys():
            ...
            diff_loss, pixel_loss, ssim_loss = self.cal_diff_loss(p_e_batch, g_e_batch)
