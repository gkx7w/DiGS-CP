...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_label(self, **kwargs):
        ...
        # 准备计算target_boxes所需的参数
        feature_map_shape = anchors.shape[:2]
        
        # 将拉平的anchors恢复为原始形状，计算每个anchor的对角线长度
        anchors_flat = anchors.reshape(-1, 7)
        anchors_d = np.sqrt(anchors_flat[:, 4] ** 2 + anchors_flat[:, 5] ** 2)
        
        # 找出所有正样本的位置
        pos_indices = np.where(label_dict['pos_equal_one'] > 0)
        num_pos = len(pos_indices[0])
        
        if num_pos == 0:
            # 如果没有正样本，返回空的target_boxes
            label_dict['target_boxes'] = np.zeros((0, 7))
            label_dict['target_positions'] = np.zeros((0, 3), dtype=np.int32)
            return label_dict
        
        # 提取特征图索引和锚框索引
        feat_h_indices, feat_w_indices, anchor_indices = pos_indices
        
        # 计算锚框在展平后的索引
        anchor_flat_indices = (feat_h_indices * anchors.shape[1] * self.anchor_num + 
                            feat_w_indices * self.anchor_num + 
                            anchor_indices)
        
        # 选择对应的锚框和锚框对角线长度
        selected_anchors = anchors_flat[anchor_flat_indices]
        selected_anchors_d = anchors_d[anchor_flat_indices]
        
        # 提取目标回归参数
        target_params = np.zeros((num_pos, 7))
        for i in range(num_pos):
            h_idx, w_idx, a_idx = feat_h_indices[i], feat_w_indices[i], anchor_indices[i]
            for j in range(7):
                target_params[i, j] = label_dict['targets'][h_idx, w_idx, a_idx * 7 + j]
        
        # 根据回归目标和锚框计算实际目标框
        target_boxes = np.zeros_like(selected_anchors)
        
        # 复原中心坐标 (x, y, z)
        target_boxes[:, 0] = target_params[:, 0] * selected_anchors_d + selected_anchors[:, 0]
        target_boxes[:, 1] = target_params[:, 1] * selected_anchors_d + selected_anchors[:, 1]
        target_boxes[:, 2] = target_params[:, 2] * selected_anchors[:, 3] + selected_anchors[:, 2]
        
        # 复原尺寸 (h, w, l)
        target_boxes[:, 3] = np.exp(target_params[:, 3]) * selected_anchors[:, 3]
        target_boxes[:, 4] = np.exp(target_params[:, 4]) * selected_anchors[:, 4]
        target_boxes[:, 5] = np.exp(target_params[:, 5]) * selected_anchors[:, 5]
        
        # 复原方向 (theta)
        target_boxes[:, 6] = target_params[:, 6] + selected_anchors[:, 6]
        
        # 存储特征图位置
        target_positions = np.stack([feat_h_indices, feat_w_indices, anchor_indices], axis=1)
        
        # 将结果添加到标签字典中
        label_dict['target_boxes'] = target_boxes
        label_dict['target_positions'] = target_positions
    ...
    def delta_to_boxes3d(deltas, anchors, channel_swap=True):
        ...
        # Inv-normalize to get xyz
        # print(deltas.device,anchors_d.device,anchors_reshaped.device)

        # print("pvco报错：",deltas.shape, anchors_d.shape, anchors_reshaped.shape)
        boxes3d[..., [0, 1]] = torch.mul(deltas[..., [0, 1]], anchors_d) + \
                               anchors_reshaped[..., [0, 1]]
        boxes3d[..., [2]] = torch.mul(deltas[..., [2]],
                                      anchors_reshaped[..., [3]]) + \
