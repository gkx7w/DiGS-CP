...
class FpvrcnnPostprocessor(VoxelPostprocessor):
    def __init__(self, anchor_params, train):
        ...
        self.iou_stage1 = True if 'iou_stage1' in anchor_params and anchor_params['iou_stage1'] else False
    ...
    def post_process_stage1(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            # print("stage1 score mask ",boxes3d.shape,scores.shape)

            dir_labels = torch.max(dir, dim=-1)[1]
            dir_labels = dir_labels[mask]

            if scores.shape[0] != 0:
                if 'iou_preds' in preds_dict and  self.iou_stage1:
        ...
        for n in batch_num_box_count:
            ...
            batch_pred_boxes3d.append(cur_boxes[keep_index].detach())
            batch_scores.append(cur_scores[keep_index].detach())
            # batch_pred_boxes3d.append(cur_boxes[keep_index])
            # batch_scores.append(cur_scores[keep_index])
            cur_idx += n

        # print("stage1 nms: ")
        # print([t.shape for t in batch_pred_boxes3d],[t.shape for t in batch_scores])
        # print(batch_scores)
