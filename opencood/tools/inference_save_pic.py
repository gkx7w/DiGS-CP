...
def test_parser():
    ...
    #                     default="/home/ypy/projects/CoAlign/checkpoints/fpco_dairv2x/stage1_633/pro_best",
    #                     help='Continued training path')
    # opv2v
    parser.add_argument('--model_dir', type=str,
                        default="/home/ypy/projects/CoAlign/checkpoints/fpco_opv2v/stage1_928/pro_best/iou_reg+",
                        help='Continued training path')
            ...
            for i, batch_data in enumerate(data_loader):
                ...
                with torch.no_grad():
                    ...
                    if (i % opt.save_vis_interval == 0) and (pred_box_tensor is not None) and (use_laplace is False):
                        ...
                        # opv2v [-140.8,-40,-3,140.8,40,1]
                        # dairv2x [-100.8,-40,-3.5,100.8,40,1.5]
                        # print(hypes['postprocess']['gt_range'])
                        # hypes['postprocess']['gt_range'] = [-60.8,-40,-3,60.8,40,1] if ("OPV2V" in hypes['test_dir']) else [-60.8,-40,-3.5,60.8,40,1.5]
                        # # 这个range也可以调整一下，因为gt在边缘是比较少的，主要看[80,80,30,30]吧

                        # 这个要保留，他看的是场景级
                        simple_vis.visualize_color(infer_result,
                                             batch_data['ego']['origin_lidar_for_vsa_project'],
                                             batch_data['ego']['batch_size'],
                        ...
                        color_index = []
                        print("场景下车辆：",batch_data['ego']['batch_size'])
                        for now_car in range(batch_data['ego']['batch_size']):
                            vis_save_path = os.path.join(vis_save_path_root, '3d_'+str(now_car)+'_%05d.png' % i)
                            # opv2v [-140.8,-40,-3,140.8,40,1]
                            # dairv2x [-100.8,-40,-3.5,100.8,40,1.5]
                            # 这个range也可以调整一下，因为gt在边缘是比较少的，主要看[80,80,30,30]吧

                            # 这个index外面给的，也不全对，外面给的可能有问题，内部解决

                            # index = batch_data['ego']['origin_lidar_for_vsa_project'][:, 0].long() == i
                            # # 不止这个，还要运算车
                            # color_index.append(index.cpu().numpy())

                            # 只显示框里面的点
                            # simple_vis.visualize_one_color(infer_result,
                            #                            batch_data['ego']['point_coords'][now_car],
                            #                            # color_index[i],
                            #                            now_car,
                            #                            hypes['postprocess']['gt_range'],
                            #                            vis_save_path,
                            ...
                            show_data = batch_data['ego']['origin_lidar_for_vsa_noproject'][:, 0].long() == now_car

                                        # 显示所有点，不显示框
                            simple_vis.visualize_one_color(infer_result,
                                                           batch_data['ego']['origin_lidar_for_vsa_noproject'][show_data][:,1:],
                                                           # color_index[i],
                                                           now_car,
                                                           hypes['postprocess']['gt_range'],
                                                           vis_save_path,
