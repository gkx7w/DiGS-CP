...
def boxes_to_corners_3d(boxes3d, order):
    ...
    # print("旋转前维度  ：",boxes3d.shape,corners3d.shape)
    # print("旋转函数维度   ：",common_utils.rotate_points_along_z(corners3d.view(-1, 8, 3),
    #                                                boxes3d_[:, 6]).shape)
...
def generate_grid(input_tensor):
    # input_tensor shape is (n, 6)
    n = input_tensor.shape[0]

    # 创建一个从 0 到 1 分为 6 个步长的一维张量
    steps = torch.linspace(0, 1, 6).to(input_tensor.device)
    # print(steps)

    # 初始化一个空列表来保存所有样本的网格坐标
    coordinates_list = []
    ...
    for sample in input_tensor:
        ...
        # 创建网格
        X, Y, Z = torch.meshgrid(step_x, step_y, step_z)
        #             print(X,Y,Z)
        # 将网格转换为 (216, 3) 的张量
        sample_coordinates = torch.stack([X.flatten(), Y.flatten(), Z.flatten()], dim=1)

        # 将单个样本的网格坐标添加到列表中
        coordinates_list.append(sample_coordinates)

    # 将所有样本的网格坐标堆叠成一个 (n, 216, 3) 的张量
    coordinates = torch.stack(coordinates_list, dim=0)

    return coordinates


def boxes_to_more_points_3d(boxes3d, order):
    """
        4 -------- 5
       /|         /|
      7 -------- 6 .
    ...
    Returns:
        corners3d: np.ndarray or torch.Tensor
        (N, 6*6*6, 3), the 8 corners of the bounding box.
    ...
    box_start_point = boxes3d.clone()
    # 首先让box点到左下角，但是必须要保持关于原点的对称关系
    box_start_point[:, 0:3] =  - box_start_point[:, 3:6]/2

    grid_points = generate_grid(box_start_point[:,0:6])
    ...
    corners3d = common_utils.rotate_points_along_z(grid_points, boxes3d_[:, 6])
